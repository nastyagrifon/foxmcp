...
### Fixed
- **`websockets` is pinned to `>=15.0,<16`.** The unbounded `>=12.0` range let a fresh install pick a version that breaks the extension socket silently: with websockets 17.x the extension's connection opens and then exchanges no messages at all before dying, which reads as a server that never answers. websockets 13.x in turn crashes the server at startup, because `websockets.serve()` no longer accepts the `process_response` hook the origin check passes. 15.0.x is the range verified end to end, with the extension connected and tool calls answered over a live socket. The resolution failure modes are documented inline in `server/requirements.txt`.
